...
async function simulateAndSend(calldata: string, value: string, label: string): Promise<string> {
  ...
  try {
    ...
    throw new Error(`simulation ${label} v4 revert: ${((e as any).shortMessage || (e as Error).message || "").slice(0, 140)}`);
  }
...
export async function closeV4Position(tokenId: string, reason?: "TP" | "SL" | "OOR" | "VFADE" | "FVLOW" | "manual"): Promise<V4CloseResult> {
  ...
  try {
    ...
    log.warn(`force-close #${tokenId}: forfeited ${forfeited} (token blocks transfer/honeypot), ETH salvaged`);
  }
  ...
  if (valuationBroken) log.warn(`#${tokenId} ${pair}: valuation broken (out=${outEth} fee=${feeEth} basis=${basisEth}) → pnl recorded as null, legs zeroed`);
  ...
  try {
    ...
    log.warn(`failed to write ledger v4 #${tokenId}: ${(e as Error).message.slice(0, 80)}`);
  }
...
export async function compoundV4Position(tokenId: string): Promise<V4CompoundResult> {
  ...
  if (isNativeCurrency(c0) || isNativeCurrency(c1)) {
    return { compounded: false, reason: `pair ${natSym()} native (compound only for ERC20 pairs)` };
  }
  ...
  if (fee0 <= 0n && fee1 <= 0n) return { compounded: false, reason: "no fees collected" };
